...
const UserPage = (): JSX.Element => {
      ...
      <div className="flex flex-row">
        <FormControl size="small">
          ...
          <Select
            ...
            label="Age"
            onChange={(val) =>
              setFilter({ ...filter, limit: Number(val.target.value) })
            }
          >
        ...
        <Pagination
          className="flex items-tend"
          count={getUsersResponse.data?.data.paging.totalPage || 0}
          shape="rounded"
          onChange={(event, page) => setFilter({ ...filter, page })}
          page={filter.page}
        />
      </div>
    );
  ...
  return (
    <div className={`page-user ${users.length === 0 && "h-96"}`}>
      <DataGrid
        rows={users}
        columns={columns}
        loading={getUsersResponse.isLoading}
        slots={{
